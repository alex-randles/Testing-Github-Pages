...
function afterInitForm() {
  ...
  if ($('#gwsEmbeddedForm').length) {
	window.setInterval(pingKeepAliveInput, 240000);
  }
}

$(window).load(function() {
  $("#EDGE_CONNECT_PHASE").css("opacity", "1").hide().fadeIn(200, function() {
	if($(".boi-skip-focus-input").length == 0){
...
$(function() {
  ...
  if (ua.match(/(iPad|iPhone|iPod)/g)) {
	...
	// Specific work-around to fix VoiceOver interaction with the Direct Debit Need a hand section
	$('#TAB_DE77623020FEEB613363783').attr('role', 'button');
  }
      ...
      $(document).swipe({
        swipeRight: function(event, direction, distance, duration, fingerCount, fingerData) {
          if (fingerData[0].start.x < 350) {
            ...
            setTimeout(function() {
              $('html').removeClass('tc-sidebar-anim');
            }, 200);
...
var TabModule = ( function() {
  ...
  function __updateCurrentTabBar(event, $header_wrapper) {
	  ...
	  //Changes done to avoid undefine console error if not using td-tab-section design
	  var headerElement	= $header_wrapper.find(".tc-tab-header-selected");
	  if(!typeof(headerElement)=="undefined"){
      var headerOffset  = $headerElement.position().left;
	  }
